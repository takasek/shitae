...
- `docs/examples/*.shitae` は parser 統合テスト（`packages/parser/test/integration.test.ts`）のフィクスチャを兼ねる。example を変えたら期待値を追随させ `pnpm -r test` で確認する（理由: docs-only のつもりの変更がテストを割る。「docs のみ」というスコープはこの repo に存在しない）。
