...
- `goto(mod::##v)` は**新規診断コード E031** を割り当てる（T4 実装時に設計者確認のうえ確定）——裸 `##v`
  の `goto` は合法（variant change）だが module 修飾つきは「mod のアクティブ component」が定まらないため
  不正。E023（フレーム新規作成系限定の条件文）を拡張すると前提文言と矛盾し、E016（back 専用・意味論が
  異なる）を流用するのも筋が悪いため、専用コードを新設する——診断表の条件文を汚さず驚きが最小になる。
- `push(mod::##v)`（**session なし**。フレームを新規作成せず現在フレームに積むだけの形）も **E031** の
  対象に含める（T4 実装中に発見された未決事項を設計者確認のうえ追加確定）。SPEC L360「session なしの
  `push(##v)` は E023 の対象外」は module 修飾なしの場合の話であり、module 修飾つきでは goto と同じ理由
  （「mod のアクティブ component」が定まらない）が push(session なし) にもそのまま当てはまるため、E031
  の条件文を「`goto(mod::##v)` / `push(mod::##v)`（session なし）」に広げる。
...
SPEC モジュール化節に 1 文足すのみ。doc common はアクティブ画面のファイル単位（SPEC「document common」節
L79 の既定の意味論。ADR-0016 B2 はこの一般規則自体の出典ではなく、未定義 component 時の帰属という
狭いエッジケースの決定）という既定の帰結であり、専用記法・エントリファイル特例は「ファイル＝名前空間」の
原則を崩すため見送る。
