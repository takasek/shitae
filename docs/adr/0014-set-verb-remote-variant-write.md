...
- **モジュール修飾**: `set(mod::X##v)` は (mod, X) の共有レジストリを書く——キー規則は **ADR-0013 のとおり**（設計上の意図としてここで明示する）。
- **診断**:
  - E029（parser）: `set` の引数が `component##variant` の形でない（`set()` / `set(X)` / `set(##v)` / `set(X, @S)` / `set(*X##v)`）。collection の `*` 誤用も E021/E024 のような専用コードは立てず、この形状包括コードで扱う（set の誤用形は多様で、1 コード 1 メッセージで「component##variant の形」へ誘導するのが読者に速いため）。
  - E030（checker）: `set` の対象が**定義済みの非 singleton** component。未定義 component への set は従来方針どおり素通り（ラフさ優先。lint はヒントを出してよい）。
  - W105 拡張: `set(X##v)` の v が X に定義されていない variant。新コードを立てず W105 を拡張するのは、指摘の本質（**未定義 variant への参照**）が goto と同一で動詞だけが違うため——E020→E027 の分割（本質が「variant 指定不要」と「母体なし」で異なる）とは逆のケースであり、「本質が同じなら同コード・違うなら分ける」の同一原則の適用である。
...
- **gate の条件源分離**（`タップ(ボタン ? ハート.残あり)` 等・findings C2）: ガード式への半歩で「条件を評価しない」原則と衝突。C2 の摩擦の本体は「variant 分割は正しいイディオムだが、その variant を条件源の変化に追随させる書き込み手段が無い」ことであり、set の導入で解消する。gate 自体を現状維持とする決定の**正典は ADR-0016 の C2 項**（SPEC 明記の文言もそちらに従う）。
