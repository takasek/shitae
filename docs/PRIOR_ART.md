...
要約すると、shitae の構成要素（UI の概念、LLM 向けの設計原理、ナビゲーションの語彙、テキスト作図、UX フロー記述）はいずれも既存技術に対応物がある。一方で、それらを「コード生成ではなくラフスケッチ」「ビジュアルではなくテキスト」「LLM の出力先ではなく人間と AI の協働の媒体」、そして「書き味はラフのまま、遷移の意味論（フレーム木）を診断・実行で確かめられる」という方向で組み合わせた例は、調査の範囲では見当たらなかった。なお shitae が持つのは診断（check）とシミュレーション（simulate）であり、系譜 A の形式検証（モデル検査）とは別物である。
