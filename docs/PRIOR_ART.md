...
さらに、shitae はこの立ち位置（ラフ記述言語）を保ったまま検証可能性を追加できた。既存の系譜では両者はトレードオフとして現れてきた。検証を持つもの（系譜 A の形式検証）は形式仕様を要求してラフさを捨て、ラフさを許すもの（系譜 D・E）は検証を持たない。shitae は書き味をラフに保ったまま（穴・未定・自然文を許したまま）、遷移の意味論（フレーム木）を診断（check）と実行（simulate）で確かめられる。なおこれはモデル検査のような形式検証（系譜 A）とは別物である。
